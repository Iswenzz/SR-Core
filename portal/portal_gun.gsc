...
stop_all( delete_portals, disconnected )
{
	if( delete_portals )	//player has died or dced
	{
		...
		for( i = 0; i < self.turrets.size; i++ )
			self.turrets[i] portal\turret::delete_turret();
...
turret()
{
	...
	else
	{
		...
		self notify( "place_turret" );
		self.turrets[self.turrets.size] = self thread portal\turret::spawn_turret( self.test_turret.origin, self.test_turret.angles );
		self.test_turret delete();
	}
...
test()
{
	...
	/*if( !isdefined( level.turret ) )
		level.turret = portal\portal::spawn_portal_object( "turret", (0,0,0), (0,0,0), "cylinder", 10, 0.3, 0.25, true, 0.1, 20 );
