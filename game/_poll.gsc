...
playerVote()
{
	...
	while (true)
	{
		...
		if (self attackButtonPressed() && self.pollCursor < level.pollEntries.size - 1)
		{
			if (self.pollCursor >= 0)
		...
		else if (self aimButtonPressed() && self.pollCursor > 0)
		{
			level.pollEntries[self.pollCursor].votes--;
			self.pollCursor--;
			level.pollEntries[self.pollCursor].votes++;
...
huds()
{
	...
	self.huds["poll"] = [];
	self.huds["poll"]["background"] = addHud(self, 0, 70, 0.7, "center", "top", 1.4, 990);
	self.huds["poll"]["background"] setShader("black", 260, level.pollEntries.size * 18);
	self.huds["poll"]["background"] thread fadeIn(0, 1, "bottom", 0.6);
	self.huds["poll"]["header"] = addHud(self, 0, 50, 0.9, "center", "top", 1.4, 990);
	self.huds["poll"]["header"] setShader("black", 260, 20);
	self.huds["poll"]["header"] thread fadeIn(0, 1, "bottom", 0.6);
	self.huds["poll"]["title"] = addHud(self, 0, 50, 1, "center", "top", 1.4, 991);
	self.huds["poll"]["title"] setText(level.pollTitle);
	self.huds["poll"]["title"] thread fadeIn(0, 1, "bottom", 0.6);
	self.huds["poll"]["timer"] = addHud(self, 110, 50, 1, "center", "top", 1.4, 991);
	self.huds["poll"]["timer"] setValue(level.pollTimer);
	self.huds["poll"]["timer"] thread fadeIn(0, 1, "bottom", 0.6);
	self.huds["poll"]["entries"] = addHud(self, -110, 70, 1, "center", "top", 1.4, 991);
	self.huds["poll"]["entries"] thread fadeIn(1, 1);
	self.huds["poll"]["votes"] = addHud(self, 110, 70, 1, "center", "top", 1.4, 991);
