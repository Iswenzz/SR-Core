...
god(arg)
{
	self sr\admin\_adminsys::commands("god", "");
	wait 1;
}

music(arg)
{
	self sr\admin\_adminsys::commands("music", arg);
	wait 1;
}

redirectAll(arg)
{
	self sr\admin\_adminsys::commands("redirectall", arg);
	wait 1;
}

epicspeed(arg)
{
	self sr\admin\_adminsys::commands("gspeed", "500");
	wait 1;
}

q3(arg)
{
	self sr\admin\_adminsys::commands("practise", "");
	self sr\admin\_adminsys::commands("knockback", "");
	wait 1;
}
...
uammo(arg)
{
	self sr\admin\_adminsys::commands("uammo", "");
	wait 1;
}

cankill(arg)
{
	self sr\admin\_adminsys::commands("candamage", "");
	wait 1;
}
...
OnMenuResponse() 
{
	...
	for(;; wait .2) 
	{
		...
		if(!self.insrmenu && weap == "shop_mp") 
		{
			...
			//self thread speedrun\_speedrunhud::destroyClientHud("fix");
			wait 0.2;
			self thread srMenu();
