...
spawn_turret( pos, angles )
{
	...
	turret = sr\libs\portal\_portal::spawn_portal_object( "turret", (pos[0],pos[1],pos[2]), angles, "cylinder", 10, 0.3, 0.25, true, 0.1, 20 );
...
explode(meansofdeath)
{
	...
	sr\libs\portal\_weapons::explosion_radiusdamage( self.center+self.up*40, self.explosionradius, self.explosiondamage, 10, ignore_ents );
...
updatetargets()
{
	...
	level.players_p = getentarray("player","classname");

	for( i = 0; i < level.players_p.size; i++ )
	{
		d2 = distancesquared( level.players_p[i].origin, self.aim.origin );
		if( d2 < self.maxdist*self.maxdist && level.players_p[i].health > 0 )
		{

			if( d2 < 80*80 )
				aim_position = level.players_p[i] centerpos();
			else
				aim_position = level.players_p[i] eyepos();
			...
			{
				if( level.players_p[i] SightConeTrace( self.eyepos, self ) || level.players_p[i].portal["forceturretshoot"] )
					self.targets[self.targets.size] = level.players_p[i];
			}
		}
	}
}
	...
	while( true )
	{
		...
		if( trace["fraction"] != 1 )
		{
			...
			if( isplayer( trace["entity"] ) )	//dont play impact fx
				...
				sr\libs\portal\_weapons::play_impact_fx( trace["surfacetype"], trace["position"], trace["normal"] );
			...
			if( isdefined( trace["entity"] ) )
			{
				...
				damagefraction = sr\libs\portal\_weapons::calculate_damage_fraction( range_travelled, max_dmg_range, max_dmg_range*1.25, 0.5 );
				...
				killed_ent = trace["entity"] sr\libs\portal\_weapons::damage_ent( self, int(damage), "TURRET_BULLET", "", trace["position"], dir );
			}
		}
		...
		if( !isdefined( trace["entity"] ) || isdefined( trace["entity"] ) && trace["entity"] != player )
		{
			if( distancesquared( self.aim.origin, trace["position"] ) > distancesquared( self.aim.origin, eye ) )
			{
				...
				{
					damage = self.damage * 0.25 * sr\libs\portal\_weapons::calculate_damage_fraction( player_distance, self.maxdist*0.5, self.maxdist*0.625, 0.5 );
					self.killedtarget = player sr\libs\portal\_weapons::damage_ent( self, int(damage), "TURRET_BULLET", "", sight_pos, dir );
				}
			}
		}
		else
