...
startvote(vote, map)
{
	...
	switch (vote)
	{
		case "map": string = map; break;
		case "endmap": string = "End map"; break;
		case "krispy": string = "^1SR-C ^7" + level.krispymap; break;
		case "add10": string = "Time +10"; break;
		case "add20": string = "Time +20"; break;
		case "msg": string = map; break;
	}
	...
	for(i=0; i<players.size; i++)
	{
		...
		players[i] thread vote_hud(string);
	}
	...
	if(level.votesYes > level.votesNo)
	{
		...
		{
			case "endmap":
				thread braxi\_mod::endMap( "Game has ended" );
				break;
			case "map":
				thread braxi\_mod::endMap( "Game has ended", map );
				break;
			case "krispy":
				thread braxi\_mod::endMap( "Game has ended", level.krispymap );
				break;
			case "add10":
				time = level.sr_time;
				time += 600;
