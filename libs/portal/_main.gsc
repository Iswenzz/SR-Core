...
onPlayerConnect()
{
	...
	while( true )
	{
		...
		player thread portal\_portal_gun::main();
		...
		//player thread portal\_daycycle::onPlayerDisconnect();
...
load_settings()
{
	...
	if( stat == 1 || ( getdvarInt( "portal_daycycle_default_on" ) && !stat ) )
		self thread portal\_daycycle::On();
	else
		self thread portal\_daycycle::Off();
